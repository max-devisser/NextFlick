...
/**
 * Builds a movie from the TMDB api
 */
public class MovieParser {
	...
	// Note: w500 is a pretty high quality image, this should be downgraded
	...
	private String grabParentalRating(JsonSearcher search) {
		...
		for (String s : certs) {	// the "releases" part of the JSON response was confusing so I just 
									// made it grab all of the different certifications and look for an MPAA rating
	 ...
	 **/
	private double grabCriticalRating(JsonSearcher search) {
		search.resetIndex();
	...
	/**
	 * Grabs Actor List from JSON List will contain no more than 10 actors
	 **/
	...
	/**
	 * Grabs country from JSON The first country listed often was not US even
	 * for US productions, so I biased this toward being US
	 **/
	...
	/**
	 * Grabs language from JSON NOTE: this might give unintended results,
	 * possibly might need fixing
	 **/
	 ...
	 * 
	 * @param id
	 *            The TMDB id of the movie to be constructed
	 * @param append
	 *            What fields to append to response, separate commands separated
	 *            by commas, should always at least include "credits,releases"
	 * @return Creates request URL and passes to constructMovieByURL, then
	 *         returns result
	 **/
	...
	/**
	 * Inputs default "credits,releases" value to the append parameter,
	 * otherwise identical to the other version
	 * 
	 * @param id
	 *            The TMDB id of the movie to be constructed
	 * @return Creates request URL and passes to constructMovieByURL, then
	...
	/**
	 * Uses TMDB search function to search the given query and gives you the top
	 * result - NOT GUARANTEED TO BE WHAT YOU'RE LOOKING FOR
	 * 
	 * @param query
	 *            The movie title you're searching for, formatted in URL style
	 *            e.g. "Batman%20the%20dark%knight"
	 * @return Returns TMDB ID of the top result for your query. If there are no
	 *         results, throws exception and stops execution
	 **/
